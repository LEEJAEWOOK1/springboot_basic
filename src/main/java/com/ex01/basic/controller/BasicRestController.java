...
public class BasicRestController {
    ...
    public ResponseEntity<List<BasicDto>> getMembers(){
        return ResponseEntity.ok(list); //현재상태를 보내준다.
    }
    ...
    public ResponseEntity<BasicDto> getMembers(@PathVariable("username") String username){
        BasicDto basicDto = list.stream()
                ...
                .orElse(null);
        if(basicDto == null){
            return ResponseEntity.status(HttpStatus.NOT_FOUND).build(); //돌려줄 데이터가 없으면 build()
        }

        return ResponseEntity.ok(basicDto); //현재상태를 보내준다.
    }
    ...
    public ResponseEntity<String> postMembers(@RequestBody BasicDto basicDto){
        ...
        if(bool) // 상태 출력
            return ResponseEntity.status(HttpStatus.CREATED).body("추가 성공"); //201 //돌려줄 값이 있으면 body()
        return ResponseEntity.status(HttpStatus.BAD_REQUEST).body("문제 발생:잘못된요청"); //400
    }
    ...
    public ResponseEntity<String> getTest01(){
        return ResponseEntity.ok("통신 성공"); //현재상태를 보내준다.
    }
    //get : 데이터 요청
